...
export default Screen = ({ name, navigation }) => {
    ...
    async function handleSubmit(){
        ...
        var { data } = await api.post(Platform.OS == 'android' ? 'service': `service?tipo=4&valores={"numero_conta": "${conta}", "data": "${dataFormatada}", "descricao": "${descricaoAtual}", "valor": "${valorAtual}"}&id=2`, '', {
            headers: {
        ...
        <KeyboardAvoidingView enabled={true} behavior='padding' style={styles.container}>
            ...
            <View style={{flexDirection: 'row', paddingLeft: 10, paddingRight: 10}}>
                ...
                <TouchableOpacity onPress={handleSubmit} style={[styles.btEnviarIOS, {marginLeft: 5}]}>
                    <Text style={{color: '#fff', fontWeight: 'bold', fontSize: 20}}>Creditar</Text>
                </TouchableOpacity>
            </View>
